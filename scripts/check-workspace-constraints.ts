...
// 私有装配目录 packages/factory/* 不入发布成员不变量——政策来源 DESIGN P-8/主线 2026-09-15 裁定，后续批次加工件循此。
const standardReleaseMemberDirectory = new RegExp(String.raw`^(?:packages\/(?!experimental\/|factory\/)[^/]+\/[^/]+|apps\/(?!desktop(?:-host)?$)[^/]+|vendor\/[^/]+)$`)
