...
const MODULE_RESOLUTION_ERROR_MESSAGE = /(?:cannot find (?:module|package)|failed to fetch dynamically imported module)/i;
/**
 ...
 */
export async function probeSymbol(specifier, symbol, shape) {
    try {
        const namespace = (await import(__rewriteRelativeImportExtension(specifier)));
        ...
        return { present: true, value: value, reason: undefined };
    }
    catch (error) {
...
export function memberOf(namespace, symbol) {
    if (!(symbol in namespace)) {
        return undefined;
    }
 ...
 */
export async function versionOf(packageName) {
    try {
