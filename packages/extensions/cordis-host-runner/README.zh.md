...
注册表就是进程内存，也是存活定义的唯一真源。会话日志只承载一次 define 调用的元数据，绝不承载它的代码：因此进程重启后确实没有任何定义，这是合理的；而 id 已无法解析的卡片会如实说明这一点，不会假装自己还能运行。本包不会自动恢复任何定义；刷新过的页面手上什么都没有，直到有人再次运行某个包——正是这一步让它绑定存活的 host 半并重新取回浏览器半。

从内存通向磁盘的写路径有且仅有一条，而且它不是恢复：`requestExport`（模型侧的 `cordis_export`）只为该 Plugin 当前已提交成功、且仅有 host 半的 Package 准备导出，并在 `cordis/request-export` 上公布其 sha256 摘要；真正写入 `<DSH 主目录>/plugins/user-persisted/<pluginId>/` 的是 `confirmDynamicExport`——它只作为面板动词存在，绝不是 Tool——前提是有人在会话之外核对过这些摘要。产物是逐字节的源码加一份 package.json 清单：沙箱显式全禁、准入姿态为 confirm-required；任何东西都不会自动挂载它，治理管线在重启后接纳它时它也保持 disabled，直到运营者显式批准并启用。
