...
function makeHarness(options: { failCommandAt?: number; probeError?: Error } = {}): Harness {
  ...
  let commandRuns = 0
  const probeReason = options.probeError instanceof Error ? options.probeError : new Error('probe failed')
  let probe: () => Promise<void> =
    options.probeError !== undefined ? () => Promise.reject(probeReason) : () => Promise.resolve()
...
function stepNames(events: Array<Record<string, unknown>>): string[] {
  ...
  return events.map(e => `${text(e.action)}:${text(e.step)}:${text(e.outcome)}`)
}
