...
- Six artifacts are pinned (verticals + omnivision + bridge + filehub + plugin-center + autopilot); the boot posture spectrum is four mounted (omnivision + bridge + filehub + plugin-center — the latter two flipped `enabledAtBoot: true` by TC-B4-RA-2 after the full R-A preconditions went green) and two skipped (verticals, opt-in per its own package docs; autopilot, product-design default-off per ADJ-3); the remaining intake plugins are added in later batches.
