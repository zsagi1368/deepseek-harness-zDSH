...
当手机只需要"看"——跟踪进度或翻看历史会话——以只读旗标启动：

```bash
dsh web --trusted-host workstation.tail1234.ts.net --read-only
```

此时 API 网关仅提供读取：会话列表、搜索、完整历史、图片附件、子代理目录、模型目录与会话日志导出照常工作；一切变更类 RPC——发送提示词、创建/分叉/重命名会话、编辑队列、工作区/预设/设置/凭据管理——都会在网关处被拒并返回 `read-only-mode` 错误，不会到达任何实现。启动行会标注该形态（`dsh web: http://127.0.0.1:3080 (read-only)`），看一眼启动日志即可分辨服务器当前呈现的面貌。

这为离开桌面场景缩小了误触与暴露面，但**不是认证**：上文的信任栅栏仍只是抗重绑定与跨站加固，tailnet 内的任何设备依然可以读取全部会话历史。访问控制仍然是 tailnet 成员资格（见下文）。启动后的快速手工验收：在手机上打开一个会话、滚动查看历史，再向输入框发送内容——发送会以可解释的 `read-only-mode` 错误失败，而不会改动工作站上的任何东西。
...
- **所有请求都返回 `read-only-mode`**：服务器以 `--read-only` 启动；去掉该旗标重启即可恢复可写会话。
